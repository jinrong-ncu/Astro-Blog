(function () {
  ...
  var examBlueprint = { single: 60, multiple: 20, true_false: 20 };
  var examTypeOrder = ["single", "multiple", "true_false"];
  var questionPoints = { single: 1, multiple: 2, true_false: 1 };
  var examQuestionCount = 100;
  var examTotalPoints = 120;

  function questionTopic(question) {
    return question.topic || question.bankName;
  }

  if (!["all", "single", "multiple", "true_false", "short"].includes(state.questionType)) state.questionType = "all";
  if (!["all", "unanswered", "favorites", "exam"].includes(state.mode)) state.mode = "all";
  ...
  Array.from(new Set(data.questions.map(questionTopic).filter(Boolean)))
  ...
  function filteredQuestions() {
    ...
    if (state.topic !== "all") {
      questions = questions.filter(function (question) { return questionTopic(question) === state.topic; });
    }
    if (state.query) {
      var query = state.query.toLocaleLowerCase("zh-CN");
      questions = questions.filter(function (question) {
        return (question.stem + " " + questionTopic(question)).toLocaleLowerCase("zh-CN").includes(query);
      });
    }
    if (state.wrongOnly) {
      questions = questions.filter(function (question) {
  ...
  function displayedOptions(question) {
    ...
    return order.map(function (originalKey, index) {
      return { displayKey: shouldRemapKeys ? String.fromCharCode(65 + index) : originalKey, option: byKey.get(originalKey) };
    });
  }
  ...
  function render() {
    ...
    var topicLabel = questionTopic(question);
    elements.questionBankName.textContent = question.bankName + (topicLabel === question.bankName ? "" : " · " + topicLabel) + " · 第 " + question.number + " 题";
  ...
  function updateBankControls() {
    ...
    var modeLabels = { all: "全部题目", unanswered: "只看未做", favorites: "我的收藏", exam: "模拟考试（100题 · 120分）" };
  ...
  function changeBank(bank) {
    state.bank = bank;
    if (state.mode === "exam" && !createExam()) state.mode = "all";
    resetRange();
    save();
    render();
  }
